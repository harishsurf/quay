...
export function useFetchRepoPermForTeam(orgName: string, teamName: string) {
  ...
  let teamRepoPerms: ITeamRepoPerms[] = repos.map((repo) => ({
    repoName: repo.name,
  ...
  // Add last_modified key from fetch repos API to create a new teamRepoPerms array
  // const teamRepoPerms: ITeamRepoPerms[] = permissions.map((perm) => {
  //   const matchingRepo = repos.find(
  //     (repo) => repo.name === perm.repository.name,
  //   );
  //   if (matchingRepo) {
  //     return {
  //       ...perm,
  //       last_modified: matchingRepo.last_modified
  //         ? matchingRepo.last_modified
  //         : -1,
  //     };
  //   }
  //   return perm;
...
export function useUpdateTeamRepoPerm(orgName: string, teamName: string) {
  ...
  return {
    updateRepoPerm,
    errorUpdateRepoPerm,
    detailedErrorUpdateRepoPerm: detailedErrorUpdateRepoPerm as BulkOperationError<ResourceError>,
