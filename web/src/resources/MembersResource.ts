...
export async function deleteTeamMemberForOrg(orgName: string, teamName: string, memberName: string) {
  try {
    await axios.delete(`/api/v1/organization/${orgName}/team/${teamName}/members/${memberName}`);
  } catch (err) {
    console.error(`Unable to delete member for team: ${teamName}`, err);
  }
}

export async function deleteCollaboratorForOrg(orgName: string, collaborator: string) {
  try {
