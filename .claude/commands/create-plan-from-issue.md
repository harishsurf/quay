...
2. **Design the feature:**
   ...
   - which type of user and what access control they have for this feature. 
   Note: quay supports : Superuser, Read-only Superuser, Regular user (non-superuser)
   - Difference in old ui vs new ui for superuser: 
      - old ui (angular) has separate panel for all superuser related operations accessible from top right user dropdown menu
      - new ui has superuser screens in following:
         - the superuser viewable users and orgs are only visible in Organization list page alongside current logged in user and their orgs.
         - The settings column in org list page is only allowed for superuser related actions. Each of these options varies for organization vs user records. Also the settings
            column isnt visible for regular user
         - There's also a separate side nav for Superuser which shows few other superuser related pages: 
            - Service keys
            - Change log
            - Usage logs
            - Messages
            - Build logs
   - Quay supports multiple login type like: Database, OIDC, LDAP etc defined via AUTHENTICATION_TYPE config.yaml varibale. Always make sure to check for access control that
      are made available for each of the login types in Angular UI for access controls, superuser actions, password checks prompts for various login types. we need
      similar access control for react as it is done in angular
...
## LDAP logins for testing: 

- All user creds for local ldap testing are in local-dev/ldap
...
## Step 6: Document the Fix Plan

After analyzing the issue and creating the TodoList, automatically create a fix documentation file:

**Create directory structure:**
```bash
mkdir -p .claude/fixes/$ARGUMENTS
```

**Create fix.md template:**

Write to `.claude/fixes/$ARGUMENTS/fix.md` with the following structure:
...
[For features: What needs to be implemented?]

## Proposed Solution
[High-level approach to fix/implement]

## Implementation Plan
[Detailed steps from TodoList]
...
- JIRA: [Link to issue]
- Attachments: `.claude/attachments/$ARGUMENTS/` (if any)
...
**During implementation**, update this file with:
- Actual root cause findings
- Code changes made
- Test results
- Lessons learned
...
2. Download any attachments to `.claude/attachments/PROJQUAY-1234/`
3. Analyze the issue type and components
4. Create a systematic implementation plan
5. Set up a TodoList to track progress
6. **Automatically create `.claude/fixes/PROJQUAY-1234/fix.md`** with pre-filled template
7. Show summary and be ready to start implementation
